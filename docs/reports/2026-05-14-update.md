...
Tier 2 is the verification surface state survey agencies and state PI offices work from. Every row carries LEIE-portal verification URLs.
...
Virginia-attributed public CSV deliverables (one per finding; the same file shape ships for every state):
...
All MMIS-ready format. All carry LEIE / SAM / NPPES portal verification URLs. Every row's directory-side context columns travel with the claims-side number, so the headline is never read in isolation. Every state has the same set of files at `/api/v1/states/<state>/h*.csv`.
...
- **Additional per-state worked examples**: every US state already has per-state CSVs published; the roadmap calls for deeper narrative breakdowns as research expands.
