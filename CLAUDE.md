...
- **GCP cost controls (load-bearing — always follow before adding paid-service usage)**: This project enforces a hard-cap architecture for any paid API. Existing controls: (1) a `$10/mo` budget alert (`6d1efd94-3b35-4aeb-af19-bb38f3bbb03f`) emails at 50/90/100%, (2) the budget publishes to `projects/thematic-fort-453901-t7/topics/billing-alerts`, (3) `infrastructure/kill-billing-function/` deploys a Cloud Function that auto-disables billing when cost ≥ budget (see its README), (4) every BigQuery query defaults to `maximum_bytes_billed=100 GB` (~$0.50). Use `DEFAULT_MAX_BYTES_BILLED` from `frontend/src/lib/bigquery.ts` (TS) or `bq_job_config()` from `analysis/claims_sources/_cohorts.py` (Python) on any new BQ work. **All Maps/Places APIs are disabled at the project level** — do not re-enable without a deliberate, documented need.
  ...
  **Guardrails are audited, because they decay silently.** `analysis/guardrails_audit.py` checks every paid surface and exits non-zero if a control is missing: the GCP budget, the auto-disable Cloud Function, the whole Maps Platform family being off, the BigQuery byte caps on both the TS and Python sides, the rate limiter, the CI anti-pattern scanner, and the `/npi` no-live-fallback contract. It reports `?` rather than failing for checks it cannot run without credentials, so the repo-side half is enforced on every PR. Wired to `.github/workflows/guardrails.yml` (push, PR, and Monday 08:30 UTC, half an hour ahead of the weekly refresh so a missing cap is known before the job that would spend against it runs).

  **This caught a real regression on 2026-08-23.** Six billable Maps Platform APIs were enabled against the policy below and nothing had noticed: Static Maps, Roads, Routes, Street View Static, Street View Publish and Time Zone. None was referenced anywhere in the codebase; the geo search was deliberately built on the directory's own coordinates plus Census centroids so none of it is ever needed. All six are now disabled and the audit fails if any comes back.

  **The Databricks account budget cannot be set from a workspace token.** `databricks account budgets` needs account-level auth against `accounts.cloud.databricks.com`, which the workspace OAuth session does not carry. Run `databricks auth login --host https://accounts.cloud.databricks.com --account-id 14397f14-e2b8-4071-a233-25d0f2ad85c1` once, then create the budget. Until then the Databricks ceiling is the warehouse configuration alone: serverless Small, `max_num_clusters=1`, 10-minute auto-stop, which bounds idle burn but does not cap total spend.
...
| `/api/v1/exploratory/<slug>.json` | `analysis/explore_*.py` | Analysis that answered a question already on the table rather than a pre-registered hypothesis. Carries `status: "exploratory"` and a `not_preregistered` sentence, and is never called a finding. Two published so far: `credential-taxonomy-mismatch`, `specialty-context`. |
...
- **Google validates every node typed `Dataset`, not only the root**, and this has now cost two Search Console reports: "Missing field description", then "Missing field creator" and "Missing field license", each time against the nested `isBasedOn` sources while the root carried the field all along. Sources live in `SOURCE_CATALOG` in `frontend/src/components/JsonLd.tsx`, keyed by URL, each with a description, a creator and a licence. A source the catalogue cannot licence (the Capital BlueCross payer directory: CMS-9115-F obliges the payer to serve it and says nothing about reuse terms) is emitted as a `CreativeWork` instead, which Google does not validate. Federal sources use `https://www.usa.gov/publicdomain/label/1.0/`; note the CMS provider-data metastore returns `license: null` on its own datasets, which is not evidence against that, because federal works are public domain by statute rather than by declaration.
...
| `practitioner_role` | `_id, _practitioner_id, _org_id, _specialty_code, _specialty_display, _specialty_codes, _location_ids, _phone, _telecom, _active` |
| `organization_affiliation` | `_id, _org_id, _participating_org_id, _active` |

**Multi-valued flattened columns are pipe-joined**, matching the existing `_location_ids` convention. `_telecom` is `system:value` pairs in source order (`fax:4129148635|phone:4123197866`); `_address_line` joins `address.line` entries (`200 Old Pond Rd|Ste 107`). `_phone` is the first entry whose **system is `phone`**, not the first telecom entry, because fax frequently precedes phone in NDH records. Coverage on the 2026-08-20 release (`/api/v1/ndh-column-coverage.json`, regenerate with `analysis/backfill_flattened_columns.py --report-only`): practitioner `_phone` 99.97% (independently reproduces H43), organization 95.61%, practitioner_role 83.89%, **location 79.0%**. `location._position_lat/lng` is populated for 98.28% of locations, which is the only geo in the NDH. **`Location.telecom` was empty at 2026-05-08 and is not any more** — anything written around location phone being unavailable should be rechecked. Organization `_phone` fell from 99.93%, which tracks the `ein` records now being half the file.

**A flattened column that holds one value from a repeating FHIR element is a trap, and it has now cost real numbers.** `PractitionerRole.specialty` is 0..* and `_specialty_code` held the first entry; 421,613 of the 16.5M role records carry more than one, up to 17. Anything read through that column undercounted, and three published figures did: role-specialty validity (H12), internal Practitioner-to-Role agreement (H13), and the practitioners whose specialty differs by organization (89,061 measured correctly, 52,898 through the column). `_specialty_codes` is the pipe-joined companion, added 2026-08-28. `Practitioner.qualification` is 0..* too, and reading `qualification[0]` alone made the NDH look like it disagreed with the NPPES primary taxonomy 11.3% of the time; across the whole array it matches 100.0%, and what is actually true is that **neither file encodes primacy by position** (`qualification[0]` matches the NPPES true primary 88.7% of the time, and NPPES's own `slot_1` is not the true primary in 14.97% of rows). When adding a flattened column, check the element's cardinality in the IG first; when reading one, check whether it is the singular or the pipe-joined form.
...
- **The account cannot publish publicly yet.** `visibility: PUBLIC` returns "Marketplace private exchange provider cannot perform this operation"; `PRIVATE` succeeds, so it is an account tier, not a bad request. Public listings need Databricks to approve the provider application. `--publish --private` is an explicit flag and never an automatic fallback, because a public publish that quietly degrades to a private one reports success for a listing nobody can find. **Resolved 2026-09-15, and it was ours.** `visibility: PUBLIC` was accepted from 2026-09-06 and the console offered "Public Marketplace", yet for nine days the consumer API said the provider did not exist (`consumer-providers get` → "not found") and the listing "has been removed by the provider", while the same calls succeeded on any third-party listing. Databricks confirmed the allow-listing twice. The cause: **visibility is a setting on a draft; publishing is a separate `summary.status` enum (`DRAFT | PENDING | PUBLISHED | SUSPENDED`).** The console's Publish button sets it. The API create call does not, and the field is omitted from every response at its zero value, so a draft never says DRAFT anywhere you can read; it just has no `status` key. "Removed by the provider" is how a non-published listing reads from outside, and a provider with zero published listings is not in the consumer directory. `PUT` with `status: PUBLISHED` fixed both errors within seconds. `marketplace_publish.py` now sends the status, checks it on read-back, and runs the consumer-side lookups before reporting success. **Verify a public listing from the consumer side, never the provider side**, and treat an absent enum as its zero value rather than as "not applicable". Going public is a decision, not a formality, because a joint press release has its own review cycle and may have to start before the listing lands. **The "at least 10 business days" figure this repo carried was never sourced.** It was asserted here and then quoted back by four documents, which is how an unsourced number becomes a planning assumption. No public Databricks page states a partner PR lead time; `partner-marketing@databricks.com` is the contact who would know. Ask rather than plan against it.
...
distrusting one. `guardrails_audit.py` passes this check rather than reporting `?` whenever a
valid account profile exists. **Profile names have inverted and two are stale (2026-09-09):** `ainpi`
is now the *workspace* profile and `fhiriq` the *account* one, while `Vestel AI LLC`
(workspace) and `FHIR IQ` (account) are both expired. `default_profile` still points
at the stale `Vestel AI LLC`, and no script here passes `-p`, so every Databricks
script fails until the default is repointed or that profile is re-authed.
...
**The archive holds 2026-05-08 and 2026-08-20. April was retired on 2026-08-23 at the owner's request.** The April partition is deleted from the Delta tables; the source `.zst` under `frontend/data/cms-npd/` is deliberately untouched, because **CMS serves only the current export and the April release exists nowhere else**. Deleting those files is irreversible in a way dropping the partition is not. Note the endpoint de-duplication finding (5,043,524 rows falling to 1,360,585 while distinct addresses held at ~1.3M) is an April-to-May comparison, so it is documented but no longer reproducible from the published archive.

**Parquet schema drifts between releases and the loader must check for it.** The flattened columns `_address_line`, `_phone`, `_telecom` and the location `_position_lat`/`_position_lng` were added to the extractors after the April and May exports were written, so those files carried 3 to 5 fewer columns than the tables. `INSERT ... SELECT *` then fails with `DELTA_DUPLICATE_COLUMNS_FOUND`, which names the symptom and not the cause. The fix is to re-export the older release with the current extractor rather than adding NULL columns: every flattened column derives from the stored `resource` JSON, which all releases carry, so a re-export back-fills real values. A NULL meaning "not extracted at the time" is the same ambiguity the map work keeps `null` and `0.0` apart for. `load_one_release()` compares schemas **before** the DELETE; the first version deleted the partition and then discovered the INSERT could not run, which is harmless once and destroys the partition on every re-run.
...
Endpoint and Location ids are **regenerated on every export**, so a cross-release diff joined on `_id` reports 100% churn every time as an artifact of id minting. The replacement key was measured rather than asserted, and it only works for one of the two: **endpoint joins on `_address` at 100.0%**, while **Location has no reliable key in these columns** (`_name`+`_city`+`_state`+`_postal_code` matches 9.7% of May rows as stored, 73.5% after upper-casing and stripping non-alphanumerics, because CMS re-cased and re-punctuated the fields). Any Location cross-release join is lossy and must state its match rate. The Delta table comments carry both warnings, `do_share()` re-applies them every run so an edit to `TABLE_COMMENTS` reaches the published table, and the COMMENT result is state-checked because several comments contain apostrophes.

**The endpoint row count fell 73% between April and May and that is de-duplication, not removal.** Distinct `_address` values went 1,300,241 → 1,300,082, with 1,299,999 shared; rows per address went 3.88 → 1.05. The April file repeated each endpoint nearly four times, so reading 5,043,524 as a count of endpoints overcounts by ~4x. Location's duplication factor barely moved (1.81 → 1.77), so its 61% fall is *not* explained by de-duplication; because its composite key is unreliable across releases, nothing here attributes that fall to deletion rather than renaming.
...
**A third schema break in this release, found on 2026-08-21 while re-running the findings.** The provider-taxonomy coding system moved from `http://nucc.org/provider-taxonomy` to `http://hl7.org/fhir/us/ndh/ValueSet/HealthcareIndividualTaxonomyVS`, which is a **ValueSet canonical in a field FHIR defines as a CodeSystem canonical**, and `PractitionerRole.specialty` completed its move from CMS Medicare specialty codes to NUCC taxonomy codes (0 of 5,759,163 role specialties now validate against the Medicare crosswalk; 99.96% validate against NUCC). Matching the old literal returns zero rows and does not error: the first Aug run of H10-H13 reported 0 practitioners with a taxonomy code and 0% specialty validity, both artifacts of the match. Use `tax_sys()` in `analysis/h10_h13_with_crosswalk.py`, which accepts either URL. **Side effect worth knowing: the internal-consistency number this project published for months (9.1% of Practitioner-to-Role pairs agreeing) was measuring the lossy Medicare-to-NUCC crosswalk, not disagreement. Both fields speak NUCC now and agree on 5,759,067 of 5,759,162 pairs.**

**Every silent-zero failure this project has hit has the same shape.** A source changes, a download fails, or reference data goes stale; the reading code returns an empty result rather than raising; and the empty result renders as a plausible finding. On 2026-08-21 three landed at once (taxonomy system URL, H51's vendor downloads failing TLS under urllib, the NPPES snapshot). The mitigations are all the same idea and all now in the code: `h51_vendor_endpoint_attribution.py` refuses to publish when every source failed to download, `h27_pii_exposure.py` proves the container field is still present before reporting zero exposures, and `high_risk_cohort.py` refuses to score a signal whose reference data cannot answer the question. **When adding a check that can report zero, add the positive control that distinguishes "nothing there" from "not looking".**
...
**The pinned release now lives in exactly two places and they are tested against each other.** `analysis/release.py` (`CURRENT_RELEASE`) for Python, `frontend/src/lib/release.ts` for the site, asserted equal by `frontend/tests/lib/release.test.ts`, which also asserts the published `stats.json` was built against the same release. Bump both in the same commit as a reload. Fourteen more scripts were migrated off the literal on 2026-08-21; `h1_h5_*`, `export_parquet`, `landscape`, `backfill_flattened_columns`, `build_phones_per_practitioner` and `h49_*` still carry one, so migrate when touching them.

**`stats.json` has a generator: `analysis/build_stats.py`.** It had none, and the weekly workflow only stamped `commit_sha`, so every other field held whatever was typed by hand when the file was created. By 2026-08-21 it claimed release 2026-05-08, 21,693,735 resources, 12 published findings and methodology 0.6.0-draft against a warehouse holding 32.5M resources and 24 published findings. Run it after the finding scripts and before `build_manifest.py`, which reads it.

**Findings that share a slug overwrite each other, and downstream readers get the survivor's numbers.** `h9_npi_luhn.py` and `h10_h13_with_crosswalk.py` both write `npi-taxonomy-correctness.json`; the second replaces `numerator`/`denominator` with H13's Practitioner-to-Role pair counts. `stats.json` read them as NPI validation counts and published 5,275,554 flagged NPIs. H9 now also writes `findings/npi-validity-summary.json`, which nothing overwrites, and that is what to read for NPI validity.

**Reference data can be staler than the release, and that turns a check into a false alarm.** `bigquery-public-data.nppes.npi_raw` is a static snapshot whose newest enumeration is **2026-02-07**. Against the 2026-08-20 release, every provider enumerated in between reads as absent from NPPES: 245,374 of them. Eight sampled at random were all real, active providers in the live registry. `high_risk_cohort.py` now measures the snapshot's as-of date and scores `not_in_nppes` at zero while it predates the release, bucketing those rows `needs-review`; `state_findings.py` draws its per-state verification sample from the OIG LEIE instead, because the deactivated-in-NPPES signal fails the same way in the other direction (three sampled were all reactivated after the snapshot). **Before trusting any NPPES-derived signal, check `MAX(provider_enumeration_date)` against the release date.** An NPI-range test suggested the ghosts were genuine and was wrong, because CMS does not issue NPIs in one ascending sequence; only the primary-source lookup settled it.
...
| `.github/workflows/automerge.yml` | label `qa-approved`, or CI / Secret scan / anti-patterns finishing on a PR | Squash-merges a same-repo PR once it carries `qa-approved` and `Lint + unit tests`, `gitleaks` and `scan changed files` passed on its head commit. The label is the approval: apply it only after the QA agent and product review pass. Spend guardrails is not required because it audits live cloud state, not the PR. A merge made with `GITHUB_TOKEN` does not trigger push workflows, so CI does not re-run on main after an automerge; Vercel still deploys through its git integration. |
