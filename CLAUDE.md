...
- **The account cannot publish publicly yet.** `visibility: PUBLIC` returns "Marketplace private exchange provider cannot perform this operation"; `PRIVATE` succeeds, so it is an account tier, not a bad request. Public listings need Databricks to approve the provider application. `--publish --private` is an explicit flag and never an automatic fallback, because a public publish that quietly degrades to a private one reports success for a listing nobody can find. **Half-lifted, and the half that moved is the misleading one (measured 2026-09-09).** `visibility: PUBLIC` is now accepted and the console offers "Public Marketplace", but the listing is still invisible to consumers four days later. **Provider-side visibility is not the thing to check.** The consumer API is, and it says the provider does not exist: `consumer-providers get 1391b933-...` returns "Provider id ... is not found", `consumer-listings get 6cf064b7-...` returns "has been removed by the provider", and the same call against any third-party listing succeeds. So the account can set the flag while the provider entity itself never reached the public directory. Ruled out: the one private exchange (`7660becc-...`) holds no listings and the archive listing is not bound to it. **Verify a public listing from the consumer side, never the provider side** — the provider side reported success for four days. Going public is a decision, not a formality, because a joint press release has its own review cycle and may have to start before the listing lands. **The "at least 10 business days" figure this repo carried was never sourced.** It was asserted here and then quoted back by four documents, which is how an unsourced number becomes a planning assumption. No public Databricks page states a partner PR lead time; `partner-marketing@databricks.com` is the contact who would know. Ask rather than plan against it.
