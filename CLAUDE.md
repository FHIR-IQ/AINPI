...
- `/briefings/va` — Markdown-rendered State of Virginia briefing for the 2026-05-04 DMAS review meeting. Sourced from `docs/briefings/2026-05-04-virginia-state-medicaid.md` via `loadMarkdown` + `MarkdownPage` (same pattern as `/faq`). Pulls together the § 455.436 framework, VA-specific data quality numbers, the 125-NPI federally-excluded cohort, the H26 4-payer cross-reference, and Stage B roadmap.
...
| `/api/v1/states/va-cohort-critical.csv` | `analysis/build_va_briefing.py` | 131 federally-excluded VA NPIs (May release; was 125 in April) + LEIE/SAM/NPPES verification URLs (DMAS-shareable) |
