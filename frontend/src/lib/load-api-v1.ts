...
export function loadLandscape(): LandscapePayload | null {
  try {
    const raw = fs.readFileSync(path.join(PUBLIC_API_ROOT, 'landscape.json'), 'utf8');
    return JSON.parse(raw) as LandscapePayload;
  } catch {
    return null;
  }
}
...
/**
 * Per-state connectivity ledger (practitioner -> role -> org -> endpoint ->
 * vendor). Generated by `analysis/state_connectivity.py <state>`.
 *
 * Build-time filesystem read only. This file is large and the route that
 * consumes it is `force-static`, so it must never be reached from a lambda at
 * request time; see the outputFileTracingExcludes note in next.config.js.
 */
export function loadStateConnectivity(state: string): ConnectivityPayload | null {
  try {
    const raw = fs.readFileSync(
      path.join(PUBLIC_API_ROOT, 'states', `${state.toLowerCase()}-connectivity.json`),
      'utf8',
    );
    return JSON.parse(raw) as ConnectivityPayload;
  } catch {
    return null;
  }
}

/** Two-letter codes that have a published connectivity ledger. */
export function allConnectivityStates(): string[] {
  try {
    return fs
      .readdirSync(path.join(PUBLIC_API_ROOT, 'states'))
      .filter((f) => f.endsWith('-connectivity.json'))
      .map((f) => f.replace('-connectivity.json', ''))
      .sort();
  } catch {
    return [];
  }
