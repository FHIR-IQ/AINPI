...
ORGANIZATION TO ENDPOINT. Two further tables, org_endpoint and org_endpoint_summary, map FHIR endpoint URLs to organization NPIs and to the EHR vendor behind them, where either source says so. A source column says where each row came from: the directory's own managingOrganization, or the public endpoint files of eight EHR vendors. The vendor rows are each vendor's own claim; CMS has not checked them. Where the two sources name different organizations for one URL, both rows are kept. 40,005 of the 68,888 vendor rows name an organization without an NPI.
...
BEFORE YOU WRITE A DIFF. Practitioner ids embed the NPI and are stable across releases. Organization ids are stable for provider records, which are NPI-keyed; the tax-record half of that table carries a random UUID. Endpoint and Location ids are random UUIDs that CMS regenerates on every export, so joining those two on _id across releases reports 100 percent churn that did not happen. Join Endpoint on _address instead. The table comments carry the same warning and the notebook demonstrates it.
