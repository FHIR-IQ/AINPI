...
ORGANIZATION TO ENDPOINT. Two further tables, org_endpoint and org_endpoint_summary, answer which FHIR endpoint belongs to which organization NPI and which EHR vendor serves it. Each row is tagged with its source: the directory itself, or the endpoint files EHR vendors publish, which are not CMS data.
